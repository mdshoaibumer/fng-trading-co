  ...
  return (
    <main style={{ background: 'var(--bg-secondary)', minHeight: '100vh', paddingTop: 'clamp(100px, 14vh, 160px)', paddingBottom: 'clamp(48px, 8vh, 120px)' }}>
      ...
      <div className="container" style={{ marginBottom: 'clamp(40px, 8vw, 80px)' }}>
        ...
        <p style={{ textAlign: 'center', color: 'var(--text-tertiary)', fontSize: '0.8rem', marginTop: '20px', maxWidth: '640px', marginInline: 'auto', lineHeight: 1.6 }}>
          {isAr
            ? 'الأرقام تقديرات داخلية مبنية على دورة حياة حبر إيكو مقارنةً بتصنيع الخراطيش الجديدة، وتختلف حسب حجم الاستخدام.'
            : 'Figures are internal estimates based on the eco-toner lifecycle versus new-cartridge manufacturing, and vary with usage volume.'}
        </p>
      </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--primary), var(--bg-darker))', padding: 'clamp(24px, 5vw, 64px)', borderRadius: 'clamp(16px, 3vw, 32px)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="sp-cycle" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'clamp(20px, 4vw, 40px)', position: 'relative', zIndex: 2 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: 'clamp(56px, 8vw, 80px)', height: 'clamp(56px, 8vw, 80px)', borderRadius: 'var(--radius-2xl)', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', border: '1px solid rgba(255,255,255,0.2)' }}>
