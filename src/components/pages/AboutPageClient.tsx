      ...
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '32px', marginBottom: '64px' }}>
          ...
          <div style={{ background: '#fff', padding: '40px', borderRadius: '24px', border: '1px solid #EEEEEE', textAlign: isAr ? 'right' : 'left' }}>
            <Target size={32} color="#8DB833" style={{ marginBottom: '24px' }} />
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1A3D2B', marginBottom: '16px' }}>{t('missionTitle')}</h3>
            <p style={{ color: '#555', lineHeight: 1.7, fontSize: '1.05rem' }}>{t('missionDesc')}</p>
          </div>

          <div style={{ background: '#fff', padding: '40px', borderRadius: '24px', border: '1px solid #EEEEEE', textAlign: isAr ? 'right' : 'left' }}>
            <Eye size={32} color="#8DB833" style={{ marginBottom: '24px' }} />
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1A3D2B', marginBottom: '16px' }}>{t('visionTitle')}</h3>
            <p style={{ color: '#555', lineHeight: 1.7, fontSize: '1.05rem' }}>{t('visionDesc')}</p>
          </div>

          <div style={{ background: '#fff', padding: '40px', borderRadius: '24px', border: '1px solid #EEEEEE', textAlign: isAr ? 'right' : 'left' }}>
            <ShieldCheck size={32} color="#8DB833" style={{ marginBottom: '24px' }} />
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1A3D2B', marginBottom: '16px' }}>{t('leadershipTitle')}</h3>
