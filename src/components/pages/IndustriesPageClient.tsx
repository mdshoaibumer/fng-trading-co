    ...
    <main style={{ background: 'var(--bg-secondary)', minHeight: '100vh', paddingTop: 'clamp(120px, 15vh, 160px)' }}>
      ...
      <div className="container" style={{ marginBottom: '80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px' }}>
          {[
            { icon: <DollarSign size={28} color="var(--accent)" />, title: isAr ? 'صفر تكلفة للأجهزة' : 'Zero Hardware Cost', desc: isAr ? 'أنت تدفع فقط مقابل الحبر. نحن نوفر الطابعة والصيانة بالكامل.' : 'You only pay for the ink you use. We provide the printer and maintenance.' },
            { icon: <Leaf size={28} color="var(--accent)" />, title: isAr ? 'أحبار صديقة للبيئة' : 'Eco-Friendly Inks', desc: isAr ? 'تتوافق مع معايير الاستدامة لتقليل البصمة الكربونية لمنشأتك.' : 'Meets rigorous sustainability standards to lower your facility\'s carbon footprint.' },
            ...
            <Reveal key={i} delay={i * 90} style={{ display: 'flex' }}>
              <div className="card-lift" style={{
                background: '#fff', padding: '32px', borderRadius: 'var(--radius-xl)', width: '100%',
                border: '1px solid var(--light-grey)', boxShadow: '0 10px 30px rgba(0,0,0,0.02)',
              }}>
                <div style={{ width: '56px', height: '56px', borderRadius: 'var(--radius-lg)', background: 'rgba(141,184,51,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' }}>
                  {b.icon}
                </div>
