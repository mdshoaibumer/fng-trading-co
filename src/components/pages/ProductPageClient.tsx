    ...
    <main style={{ minHeight: '100vh', background: '#F8FAF7', paddingTop: '120px', paddingBottom: '80px' }}>
      <div className="container">
        ...
        <div style={{ marginBottom: '32px', display: 'flex', alignItems: 'center', gap: '16px', flexDirection: isAr ? 'row-reverse' : 'row' }}>
          ...
          <nav aria-label={isAr ? 'مسار التنقل' : 'Breadcrumb'} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#666', fontSize: '0.9rem', flexDirection: isAr ? 'row-reverse' : 'row', flexWrap: 'wrap' }}>
            <Link href={`/${locale}`} transitionTypes={['nav-back']} style={{ color: 'var(--accent-text)', textDecoration: 'none', fontWeight: 600 }}>{isAr ? 'الرئيسية' : 'Home'}</Link>
            ...
            >
              {itemType === 'printer' ? (isAr ? 'الطابعات' : 'Printers') : (isAr ? 'المعدات المكتبية' : 'Office Equipment')}
            </Link>
            <span aria-hidden="true">/</span>
            <span style={{ fontWeight: 500 }} aria-current="page">{product.name}</span>
          </nav>
        </div>
        ...
        <div className="product-grid" style={{
            ...
            >
              <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                <Image
                  ...
                  style={{
                    objectFit: 'contain',
                    transition: isZoomed ? 'none' : 'all 0.5s var(--ease-ink)',
              ...
              <div style={{
                ...
                {product.images.map((img: string, idx: number) => (
                  <button
                    ...
                    style={{
                      position: 'relative',
                      width: '80px', height: '80px', borderRadius: 'var(--radius-lg)', overflow: 'hidden',
          ...
          <div className="info-column" style={{ position: 'sticky', top: '120px', order: isAr ? 1 : 2, textAlign: isAr ? 'right' : 'left' }}>
            ...
            {/* Specifications */}
            <div style={{ background: 'white', borderRadius: 'var(--radius-xl)', padding: '32px', border: '1px solid #E0E7DE' }}>
