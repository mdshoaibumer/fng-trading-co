...
function StatCounter({ value, unit, label }: { value: string; unit: string; label: string }) {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', marginTop: '8px', fontWeight: 600 }}>{label}</div>
    </div>
  );
}
...
export default function EcoInksSustainabilitySection() {
  ...
  return (
    <section id="sustainability" className="section" style={{ background: 'var(--bg-secondary)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className="sust-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 'clamp(16px, 4vw, 40px)', marginBottom: 'clamp(40px, 8vw, 80px)' }}>
          {stats.map((s, i) => (
            <Reveal key={s} delay={i * 120} from="scale" style={{ background: '#FFFFFF', border: '1px solid var(--light-grey)', borderRadius: 'var(--radius-2xl)', padding: 'clamp(20px, 4vw, 32px) clamp(16px, 3vw, 24px)', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
          ...
          <div className="timeline-items" style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', position: 'relative' }}>
            {milestones.map((yr, i) => (
              <Reveal key={yr} delay={i * 100} from="scale" style={{ textAlign: 'center', flex: '1 1 100px', minWidth: '80px' }}>
                <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: 'var(--accent)', margin: '0 auto 10px', border: '3px solid var(--bg-secondary)', boxShadow: '0 0 0 2px rgba(141,184,51,0.3)' }} />
