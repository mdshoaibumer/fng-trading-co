          ...
          <div className="why-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(32px, 6vw, 64px)', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {POINT_KEYS.map((key, i) => (
                ...
                <div
                  className="glass"
                  style={{ padding: '20px 24px', display: 'flex', gap: '16px', alignItems: 'flex-start', flexDirection: isAr ? 'row-reverse' : 'row', textAlign: isAr ? 'right' : 'left', background: 'rgba(247,248,245,0.9)', transition: 'all 350ms var(--ease-spring)', cursor: 'default' }}
                ...
                >
                  <div style={{ flexShrink: 0, width: '44px', height: '44px', borderRadius: 'var(--radius-md)', background: 'rgba(141,184,51,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {POINT_ICONS[key]}
                  </div>
                  <div>
                    <h3 style={{ color: 'var(--primary)', fontSize: 'var(--text-base)', fontWeight: 700, marginBottom: '4px' }}>{t(`points.${key}.title`)}</h3>
