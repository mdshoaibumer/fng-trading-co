...
export default function WhatsAppButton({ whatsapp }: { whatsapp?: string }) {
  ...
  // Differentiates the two business lines this one global button covers —
  // there's no per-product context here (unlike ProductPageClient's own
  // WhatsApp CTA), but at least the sales team opening the chat knows which
  // business the visitor was browsing.
  const prefillText = isSourcing ? t('whatsappPrefillSourcing') : t('whatsappPrefillPrinter');

  return (
    <a
      href={`https://wa.me/${sanitizeWhatsappNumber(whatsapp || DEFAULT_WHATSAPP_NUMBER)}?text=${encodeURIComponent(prefillText)}`}
      target="_blank"
    ...
    >
      {/* Occasional idle ring-pulse — draws the eye to the site's primary
          conversion channel without being a constant distraction. Only while
          idle (hidden on hover, where the expanding label already carries
          the attention) and respects prefers-reduced-motion. */}
      {/* Dual radar ripple rings */}
      {!hovered && (
        <>
          <span aria-hidden="true" className="wa-pulse-ring" style={{
            position: 'absolute', inset: 0, borderRadius: 'var(--radius-pill)',
            border: '2px solid #25D366', pointerEvents: 'none',
          }} />
          <span aria-hidden="true" className="wa-pulse-ring-2" style={{
            position: 'absolute', inset: 0, borderRadius: 'var(--radius-pill)',
            border: '1.5px solid #25D366', pointerEvents: 'none',
          }} />
        </>
      )}

      {/* Floating peek tooltip for desktop */}
      {showTooltip && !hovered && !isMobile && (
        <div
          ...
          style={{
            ...
            gap: '10px',
            whiteSpace: 'nowrap',
            animation: 'fadeInUp 400ms var(--ease-spring)',
            backdropFilter: 'blur(8px)',
          }}
        >
          <span style={{ fontSize: '0.85rem', fontWeight: 500 }}>
            {locale === 'ar' ? '👋 هل لديك استفسار؟ تحدث معنا' : '👋 Have questions? Chat with us'}
          </span>
          <button
      ...
      {showLabel && (
        <span style={{ display: 'flex', flexDirection: 'column', gap: '2px', animation: 'fadeIn 200ms ease' }}>
          <span style={{ fontSize: '0.85rem', fontWeight: 600, whiteSpace: 'nowrap', lineHeight: 1.1 }}>
            {t('whatsapp')}
          </span>
          <span style={{ fontSize: '0.68rem', fontWeight: 500, opacity: 0.85, whiteSpace: 'nowrap', lineHeight: 1.1 }}>
            {t('form.response')}
          </span>
        </span>
      )}
      <style jsx>{`
        @media (prefers-reduced-motion: no-preference) {
          .wa-pulse-ring { animation: waPulseRing 6s ease-out infinite; }
          .wa-pulse-ring-2 { animation: waPulseRing 6s ease-out infinite 1.2s; }
        }
        @keyframes waPulseRing {
          0%, 65% { transform: scale(1); opacity: 0; }
