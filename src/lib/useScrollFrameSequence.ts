...
export function useScrollFrameSequence({
  ...
  useEffect(() => {
    ...
    // Reduced motion: don't scrub the sequence on scroll. Hold the assembled
    // first frame and report a static progress of 0 so any parallax/opacity
    // derived from it also rests (WCAG 2.3.3 animation-from-interactions).
    if (prefersReducedMotion) {
      currentFrameRef.current = 0;
      // A no-op when already 0 (the normal load case); a single reset when the
      // visitor enables reduced motion after scrolling. Not a cascading render.
